...
export const ClassPicker: React.FC<ClassPickerProps> = ({ value, onChange, large, disabled }) => {
  return (
    <Radio.Group
      className={classNames("class-picker-container", { "class-large-picker-container": large })}
      disabled={disabled}
      value={value}
      onChange={e => onChange && e.target.value && onChange(e.target.value as ClassPickerItemType)}
    >
