...
import type { IServiceVideoChatAvatar } from "@netless/flint-services";
...
export class RTCRemoteAvatar implements IServiceVideoChatAvatar {
  ...
  public constructor(config: RTCRemoteAvatarConfig = {}) {
    ...
    // 作用： 监听每一位远程用户的 是否开启摄像头状态，关闭或渲染对应视频通话流
    // addDisposer 接收一个副作用函数 销毁时调用 flushAll 统一销毁
    this._sideEffect.addDisposer(
      // combine 订阅多个响应式数据改变触发 回调
      combine([this._el$, this._videoTrack$, this._shouldCamera$]).subscribe(
        ([el, videoTrack, shouldCamera]) => {
          // add 执行传递的方法 接收返回的副作用函数 销毁时调用 flushAll 统一销毁
